...
func (cq *CommentQuery) QueryAccount() *AccountQuery {
...
func (cq *CommentQuery) WithAccount(opts ...func(*AccountQuery)) *CommentQuery {
	query := &AccountQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := cq.withAccount; query != nil {
		if err := cq.loadAccount(ctx, query, nodes, nil,
			func(n *Comment, e *Account) { n.Edges.Account = e }); err != nil {
			return nil, err
		}
	}
...
func (cq *CommentQuery) loadAccount(ctx context.Context, query *AccountQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Account)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Comment)
	for i := range nodes {
