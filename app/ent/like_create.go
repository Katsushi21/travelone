...
func (lc *LikeCreate) SetAccountID(u uuid.UUID) *LikeCreate {
	lc.mutation.SetAccountID(u)
	return lc
}
...
func (lc *LikeCreate) check() error {
	...
	if _, ok := lc.mutation.AccountID(); !ok {
		return &ValidationError{Name: "account_id", err: errors.New(`ent: missing required field "Like.account_id"`)}
	}
	if _, ok := lc.mutation.PostID(); !ok {
		return &ValidationError{Name: "post_id", err: errors.New(`ent: missing required field "Like.post_id"`)}
	}
	if _, ok := lc.mutation.AccountID(); !ok {
		return &ValidationError{Name: "account", err: errors.New(`ent: missing required edge "Like.account"`)}
	}
	if _, ok := lc.mutation.PostID(); !ok {
