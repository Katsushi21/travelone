...
// QueryMarker chains the current query on the "marker" edge.
func (pq *PostQuery) QueryMarker() *MarkerQuery {
	query := &MarkerQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PostQuery) QueryAccount() *AccountQuery {
...
// WithMarker tells the query-builder to eager-load the nodes that are connected to
// the "marker" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PostQuery) WithMarker(opts ...func(*MarkerQuery)) *PostQuery {
	query := &MarkerQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PostQuery) WithAccount(opts ...func(*AccountQuery)) *PostQuery {
	query := &AccountQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := pq.withMarker; query != nil {
		if err := pq.loadMarker(ctx, query, nodes, nil,
			func(n *Post, e *Marker) { n.Edges.Marker = e }); err != nil {
			return nil, err
		}
	}
	if query := pq.withAccount; query != nil {
		if err := pq.loadAccount(ctx, query, nodes, nil,
			func(n *Post, e *Account) { n.Edges.Account = e }); err != nil {
			return nil, err
		}
	}
...
func (pq *PostQuery) loadMarker(ctx context.Context, query *MarkerQuery, nodes []*Post, init func(*Post), assign func(*Post, *Marker)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Post)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Marker(func(s *sql.Selector) {
		s.Where(sql.InValues(post.MarkerColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (pq *PostQuery) loadAccount(ctx context.Context, query *AccountQuery, nodes []*Post, init func(*Post), assign func(*Post, *Account)) error {
