...
func AccountID(v uuid.UUID) predicate.Post {
	return predicate.Post(func(s *sql.Selector) {
...
func AccountIDEQ(v uuid.UUID) predicate.Post {
	return predicate.Post(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldAccountID), v))
	})
}
...
func AccountIDNEQ(v uuid.UUID) predicate.Post {
	return predicate.Post(func(s *sql.Selector) {
...
func AccountIDIn(vs ...uuid.UUID) predicate.Post {
	v := make([]interface{}, len(vs))
	for i := range v {
		v[i] = vs[i]
	}
...
func AccountIDNotIn(vs ...uuid.UUID) predicate.Post {
	v := make([]interface{}, len(vs))
	for i := range v {
		v[i] = vs[i]
	}
...
// HasMarker applies the HasEdge predicate on the "marker" edge.
func HasMarker() predicate.Post {
	return predicate.Post(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(MarkerTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2O, false, MarkerTable, MarkerColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasMarkerWith applies the HasEdge predicate on the "marker" edge with a given conditions (other predicates).
func HasMarkerWith(preds ...predicate.Marker) predicate.Post {
	return predicate.Post(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(MarkerInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2O, false, MarkerTable, MarkerColumn),
		)
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
