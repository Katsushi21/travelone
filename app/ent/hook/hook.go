...
import (
	...
	"github.com/Katsushi21/traveling_alone/ent"
)
...
// The MarkerFunc type is an adapter to allow the use of ordinary
// function as Marker mutator.
type MarkerFunc func(context.Context, *ent.MarkerMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f MarkerFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.MarkerMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.MarkerMutation", m)
	}
	return f(ctx, mv)
}
