...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	for name, query := range aq.withNamedPosts {
		if err := aq.loadPosts(ctx, query, nodes,
			func(n *Account) { n.appendNamedPosts(name) },
			func(n *Account, e *Post) { n.appendNamedPosts(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range aq.withNamedComments {
		if err := aq.loadComments(ctx, query, nodes,
			func(n *Account) { n.appendNamedComments(name) },
			func(n *Account, e *Comment) { n.appendNamedComments(name, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range aq.withNamedRequests {
		if err := aq.loadRequests(ctx, query, nodes,
			func(n *Account) { n.appendNamedRequests(name) },
			func(n *Account, e *Account) { n.appendNamedRequests(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range aq.withNamedLikes {
		if err := aq.loadLikes(ctx, query, nodes,
			func(n *Account) { n.appendNamedLikes(name) },
			func(n *Account, e *Like) { n.appendNamedLikes(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range aq.withNamedSession {
		if err := aq.loadSession(ctx, query, nodes,
			func(n *Account) { n.appendNamedSession(name) },
			func(n *Account, e *Session) { n.appendNamedSession(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (aq *AccountQuery) WithNamedPosts(name string, opts ...func(*PostQuery)) *AccountQuery {
	query := &PostQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	if aq.withNamedPosts == nil {
		aq.withNamedPosts = make(map[string]*PostQuery)
	}
	aq.withNamedPosts[name] = query
	return aq
}

// WithNamedComments tells the query-builder to eager-load the nodes that are connected to the "comments"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (aq *AccountQuery) WithNamedComments(name string, opts ...func(*CommentQuery)) *AccountQuery {
	query := &CommentQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	if aq.withNamedComments == nil {
		aq.withNamedComments = make(map[string]*CommentQuery)
	}
	aq.withNamedComments[name] = query
	return aq
}
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (aq *AccountQuery) WithNamedRequests(name string, opts ...func(*AccountQuery)) *AccountQuery {
	query := &AccountQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	if aq.withNamedRequests == nil {
		aq.withNamedRequests = make(map[string]*AccountQuery)
	}
	aq.withNamedRequests[name] = query
	return aq
}

// WithNamedLikes tells the query-builder to eager-load the nodes that are connected to the "likes"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (aq *AccountQuery) WithNamedLikes(name string, opts ...func(*LikeQuery)) *AccountQuery {
	query := &LikeQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	if aq.withNamedLikes == nil {
		aq.withNamedLikes = make(map[string]*LikeQuery)
	}
	aq.withNamedLikes[name] = query
	return aq
}

// WithNamedSession tells the query-builder to eager-load the nodes that are connected to the "session"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (aq *AccountQuery) WithNamedSession(name string, opts ...func(*SessionQuery)) *AccountQuery {
	query := &SessionQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	if aq.withNamedSession == nil {
		aq.withNamedSession = make(map[string]*SessionQuery)
	}
	aq.withNamedSession[name] = query
	return aq
}
